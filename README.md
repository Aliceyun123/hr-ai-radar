...
- **推荐理由真实化**：精选条目的一句话推荐理由由管线侧真实生成——对过了 AI 相关性筛选的条目抓取全文，交给 DeepSeek 写一句「为什么值得读」（需要 `DEEPSEEK_API_KEY`）；没有真实理由时前端直接隐藏这个区块，不再用模板句撑场面。命中当日 TOP3 的故事集中在首页「今日 TOP3 · 三口味锐评」板块置顶展示，卡片内直接展开三口味并排面板
...
- 每日精选20条按默认口味打分点评，精选卡片带一句口味锐评行；命中当日 TOP3 的故事进入首页「今日 TOP3 · 三口味锐评」板块，三味并排，一条新闻三个角度。

![今日 TOP3 三口味并排锐评截图](assets/screenshots/radar-v09-top3-personas.png)
